...
/**
 * ProductCard
 * - Click card / imagen => navega a la página del producto (/repuestos/categoria/marca/producto-id)
 * - Click Previsualizar => modal (onPreviewClick)
 */
const ProductCard = memo(({ product, onProductClick, onPreviewClick, priority = false }) => {
  ...
  }, [onProductClick, product, productUrl, navigate]);
  ...
  }, [onPreviewClick, onProductClick, product]);
  ...
  return (
    <article
      onClick={handleClick}
      className="group bg-white rounded-lg border border-gray-100 hover:border-blue-200 shadow-sm hover:shadow-xl hover:shadow-blue-500/5 transition-all duration-300 flex flex-col h-full cursor-pointer"
      role="button"
      tabIndex={0}
