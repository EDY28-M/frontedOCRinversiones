  ...
  return (
    <>
      ...
      <aside className={`w-full lg:w-64 flex-shrink-0 border-r border-gray-200 bg-white p-6 lg:min-h-[calc(100vh-80px)] ${mobileFiltersOpen ? 'block' : 'hidden lg:block'}`}>
        <div className="mb-8">
          <h3 className="text-xs font-bold tracking-widest mb-6 uppercase text-gray-500">Categorías</h3>
          {isLoadingCategories ? (
            ...
            <div className="flex flex-col gap-2">
              ...
              >
                <span className={`w-1 h-5 rounded-full transition-colors ${
                  selectedCategory === null ? 'bg-primary' : 'bg-gray-300 group-hover:bg-primary/50'
                }`}></span>
                <span className={`text-sm ${
                  selectedCategory === null ? 'font-bold text-gray-900' : 'font-medium text-gray-700 group-hover:text-gray-900'
              ...
              {categories.map(cat => (
                ...
                >
                  <span className={`w-1 h-5 rounded-full transition-colors ${
                    selectedCategory === (cat.id || cat.Id) ? 'bg-primary' : 'bg-gray-300 group-hover:bg-primary/50'
                  }`}></span>
                  <span className={`text-sm ${
                    selectedCategory === (cat.id || cat.Id) ? 'font-bold text-gray-900' : 'font-medium text-gray-700 group-hover:text-gray-900'
                  }`}>
        ...
        <div className="mb-12">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-xs font-bold tracking-widest uppercase text-gray-500">Marcas</h3>
            {selectedBrands.length > 0 && (
              <button
                onClick={() => onBrandToggle('all')}
            ...
            <div className="flex flex-col gap-2 max-h-60 overflow-y-auto pr-2">
              {brands.map(brand => (
                ...
                >
                  ...
                  <span className={`text-sm ${
                    selectedBrands.includes(brand.id || brand.Id) ? 'font-bold text-gray-900' : 'font-medium text-gray-700 group-hover:text-gray-900'
                  }`}>
        ...
        <div className="border-t border-gray-200 pt-6">
          <button
            ...
            className="w-full py-2 px-4 bg-gray-100 hover:bg-gray-200 text-gray-800 border border-gray-300 rounded transition-colors text-sm font-medium"
          >
            RESET
          </button>
        </div>
